...
kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True).add("Ответить на вопросы")
...
async def start(update: types.ChatJoinRequest):
    await bot.send_message(update.from_user.id, "Чтобы пройти в телеграм канал, пожалуйста ответьте на 3 вопросов",
                           reply_markup=kb)


async def start_que(message: types.Message):
...
async def answer3(message: types.Message, state: FSMContext):
    ...
    x = await insert_lead(answ1, answ2, answ3, message.from_user.username, message.from_user.first_name)
    if x is True:
        await state.finish()
        try:
...
def register_handlers_other(dp: Dispatcher):
    ...
    dp.register_message_handler(start_que, Text("Ответить на вопросы"))
